...
Ext.Panel=Ext.extend(Ext.Container,{baseCls:'x-panel',collapsedCls:'x-panel-collapsed',elements:'body',maskDisabled:true,collapsed:false,animCollapse:Ext.enableFx,toolTarget:'header',headerAsText:true,buttonAlign:'right',minButtonWidth:75,collapseEl:'bwrap',slideAnchor:'t',collapseFirst:true,deferHeight:true,expandDefaults:{duration:.25},collapseDefaults:{duration:.25},initComponent:function(){Ext.Panel.superclass.initComponent.call(this);this.addEvents({bodyresize:true,titlechange:true,collapse:true,expand:true,beforecollapse:true,beforeexpand:true,beforeclose:true,close:true,activate:true,deactivate:true});if(this.tbar){this.elements+=',tbar';if(typeof this.tbar=='object'){this.topToolbar=this.tbar;}
delete this.tbar;}
...
if(this.buttons){var btns=this.buttons;delete this.buttons;for(var i=0,len=btns.length;i<len;i++){this.addButton(btns[i]);}}
...
this.bwrap.enableDisplayMode('block');if(this.header){this.header.unselectable();}
if(this.header){if(this.headerAsText){this.header.dom.innerHTML='<span class="'+this.headerTextCls+'">'+this.header.dom.innerHTML+'</span>';if(this.iconCls){this.setIconClass(this.iconCls);}}}
if(this.floating){this.makeFloating(this.floating);}
if(this.collapsible){this.tools=this.tools||[];if(!this.hideCollapseTool){this.tools[this.collapseFirst?'unshift':'push']({id:'toggle',handler:this.toggleCollapse,scope:this});}
...
if(!this.toolTemplate){var tt=new Ext.Template('<div class="x-tool x-tool-{id}">&#160</div>');tt.disableFormats=true;tt.compile();Ext.Panel.prototype.toolTemplate=tt;}
for(var i=0,a=arguments,len=a.length;i<len;i++){var tc=a[i],overCls='x-tool-'+tc.id+'-over';var t=this.toolTemplate.insertFirst(this[this.toolTarget],tc,true);this.tools[tc.id]=t;t.enableDisplayMode('block');t.on('click',this.createToolHandler(t,tc,overCls));if(tc.on){t.on(tc.on);}
if(tc.hidden){t.hide();}
t.addClassOnOver(overCls);}},createToolHandler:function(t,tc,overCls){return function(e){t.removeClass(overCls);e.stopEvent();if(tc.handler){tc.handler.call(tc.scope||t,e,t);}};},afterRender:function(){if(this.fromMarkup&&this.height===undefined&&!this.autoHeight){this.height=this.el.getHeight();}
if(this.floating&&!this.hidden&&!this.initHidden){this.el.show();}
if(this.title){this.setTitle(this.title);}
if(this.autoScroll){this.body.dom.style.overflow='auto';}
if(this.html){this.body.update(this.html);delete this.html;}
...
return this.keyMap;},initEvents:function(){if(this.keys){this.getKeyMap();}},beforeEffect:function(){if(this.floating){this.el.beforeAction();}
...
this.fireEvent('titlechange',this,title);return this;},getUpdater:function(){return this.body.getUpdater();},load:function(){var um=this.body.getUpdater();um.update.apply(um,arguments);return this;},beforeDestroy:function(){Ext.Element.uncache(this.header,this.tbar,this.bbar,this.footer,this.body);},createClasses:function(){this.headerCls=this.baseCls+'-header';this.headerTextCls=this.baseCls+'-header-text';this.bwrapCls=this.baseCls+'-bwrap';this.tbarCls=this.baseCls+'-tbar';this.bodyCls=this.baseCls+'-body';this.bbarCls=this.baseCls+'-bbar';this.footerCls=this.baseCls+'-footer';},createGhost:function(cls,useShim,preventAppend){var el=document.createElement('div');el.className='x-panel-ghost '+(cls?cls:'');if(this.header){el.appendChild(this.el.dom.firstChild.cloneNode(true));}
Ext.fly(el.appendChild(document.createElement('ul'))).setSize(this.bwrap.getSize());if(!preventAppend){this.container.dom.appendChild(el);}
