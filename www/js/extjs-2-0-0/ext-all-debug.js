...
El.prototype = {
    ...
    mask : function(msg, msgCls){
        ...
        // Try to handle case of innerHTML overwrite
        if(this._mask && (!this._mask.dom || !this._mask.dom.parentNode)){
            this.unmask(true);
        }
        if(!this._mask){
...
Ext.layout.Accordion = Ext.extend(Ext.layout.FitLayout, {
    ...
    collapseFirst:false,
    animate : false, // use undefined to defer to panel specific configs
...
Ext.Panel = Ext.extend(Ext.Container, {
    ...
    addTool : function(){
        ...
        for(var i = 0, a = arguments, len = a.length; i < len; i++) {
            ...
            t.on('click', this.createToolHandler(t, tc, overCls));
            if(tc.on){
                t.on(tc.on);
            }
            if(tc.hidden){
    ...
    createToolHandler : function(t, tc, overCls){
        return function(e){
            t.removeClass(overCls);
            e.stopEvent();
            if(tc.handler){
                tc.handler.call(tc.scope || t, e, t);
            }
        };
    },
